...
```bash
git clone <this-repo> e-gov-mcp
cd e-gov-mcp
go build -o e-gov-mcp ./...
```

`go install github.com/piro-hiroki/e-gov-mcp@latest` でも導入できます（`$GOBIN/e-gov-mcp`）。
...
```bash
claude mcp add e-gov -- /path/to/e-gov-mcp/e-gov-mcp
```
...
```json
{
  "mcpServers": {
    "e-gov": {
      "command": "/path/to/e-gov-mcp/e-gov-mcp"
    }
  }
}
```
