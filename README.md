...
cd e-gov-lawapi-v2
make build      # または: go build -o e-gov-lawapi-v2 ./cmd/e-gov-lawapi-v2
```

`go install github.com/piro-hiroki/e-gov-lawapi-v2/cmd/e-gov-lawapi-v2@latest` でも導入できます（`$GOBIN/e-gov-lawapi-v2`）。
...
```
.
├── cmd/e-gov-lawapi-v2/    # バイナリエントリポイント
├── internal/
│   ├── egov/               # e-Gov 法令API v2 クライアント（再利用可）
│   └── mcpserver/          # MCP server の tool 登録
